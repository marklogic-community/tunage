...
declare namespace itunes = "http://marklogic.com/extension/plugin/itunes";
...
as empty-sequence()
{
    ...
    (: FIXME :)
		let $xml := xdmp:document-get(data($model/plugin:data/*:uri))
    
    (:
    let $validate := if(fn:empty($xml/plist)) then
    	infodev:handle-error($ticket-id, concat("Invalid iTunes metadata", xdmp:quote($xml)), ())
    	else ()
    :)
    
    let $xsl :=  
    	<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs" xmlns:t="http://marklogic.com/demo/tunes">
				<xsl:template match="/">
					<xsl:apply-templates select="plist/dict/dict[1]/dict"/>
				</xsl:template>
				...
				<xsl:template match="key"> 
					<xsl:element name="{{translate(., ' ', '_')}}" namespace="http://marklogic.com/demo/tunes">
						<xsl:value-of select="(following-sibling::*)[1]"/>
					</xsl:element>
				</xsl:template> 
			</xsl:stylesheet> 
		
		let $tracks := (xdmp:xslt-eval($xsl, $xml)/t:TRACK)(:[1 to 10]:)
    ...
    let $transactions :=
        ...
        let $put :=
            for $track as element(t:TRACK) in ($tracks)[$start to $finish]
            let $id := fn:concat($track/t:Track_ID,".xml")
...
plugin:register(itunes:capabilities(),"collector-itunes.xqy")
